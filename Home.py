...
#        st.session_state['data'] = st.file_uploader("Tournament Data")
    ...
    st.write("""After Waco, there was more data for me to use, and after requesting for a new field to be added to the dataset, who won or lost, I was able to finally approach an ML method of solving match prediction problems. With this better data, I found out how to create a dynamic feature between each team by taking their standardized scores between 0 and 1 and creating differing features that would categorize each alliance's outcome as a loss or win. In the first stages, I approached ensemble learning and found 3 different methods that work the best: logistic regression, gradient boosting, and hyperparameter tuning. After developing a clean and stable model that doesn't overfit the WACO event tournament I developed the ML model, and it gave back amazing results that I listed below.""")
