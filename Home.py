...
#        st.session_state['data'] = st.file_uploader("Tournament Data")
    ...
    st.markdown("Accuracies for the first couple of tournaments. They vary greatly compared to the accuracies from the match prediction page as I modified the model for world champs. However, the seating arangement and mid season shift in strategy caused a shift in the effectiveness in features causing the model to be accurate for previous meta play, but not for new play. This accuracy is still correct for fortworth and waco when the model was adjusted to regional play."
    ...
    st.write("""After Waco, there was more data for me to use, and after requesting for a new field to be added to the dataset, who won or lost, I was able to finally approach an ML method of solving match prediction problems. With this better data, I found out how to create a dynamic feature between each team by taking their standardized scores between 0 and 1 and creating differing features that would categorize each alliance's outcome as a loss or win. In the first stages, I approached ensemble learning and used 3 different methods: gradient boosting, hyperparameter tuning, and feature engineering. After developing a stable model that didn't overfit the WACO event tournament I trained the ML model on more generalized matches, and it gave back amazing results that I listed below.""")
